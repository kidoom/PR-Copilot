...
def verify_repo_context(
    ...
    if not remote_url:
        errors.append("Cannot read git remote origin URL")
    else:
        parsed = _parse_owner_repo_from_remote(remote_url)
        if not parsed:
            errors.append(f"Cannot parse owner/repo from remote URL: {remote_url}")
        else:
    ...
    if trusted_sha:
        ...
        if not actual_sha:
            errors.append("Cannot read HEAD commit SHA from repository")
        elif not actual_sha.startswith(trusted_sha):
            errors.append(
...
def search_repo(
    ...
    if path_scope:
        ...
        if not safe_scope:
            return {"error": "Invalid or unsafe path_scope", "path_scope": path_scope}
