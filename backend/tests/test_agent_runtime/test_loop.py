...
async def test_final_only_response():
    model = FakeModelClient([
        ModelResponse(content="final answer", tool_use_blocks=[]),
    ])
    reg = ToolRegistry()
...
async def test_tool_call_then_final():
    model = FakeModelClient([
        ModelResponse(
            ...
            tool_use_blocks=[ToolUseBlock(tool_use_id="u1", name="echo", input={"text": "hello"})],
        ),
    ...
    assert kinds == ["think", "call", "observe", "final"]
...
async def test_max_steps_stops_loop():
    model = FakeModelClient([
        ModelResponse(
            ...
            tool_use_blocks=[ToolUseBlock(tool_use_id=f"u{i}", name="echo", input={"text": str(i)})],
        )
        for i in range(5)
    ])
    reg = ToolRegistry()
...
async def test_unknown_tool_returns_error():
    model = FakeModelClient([
        ModelResponse(
            ...
            tool_use_blocks=[ToolUseBlock(tool_use_id="u1", name="nonexistent", input={})],
        ),
...
async def test_multiple_tool_calls():
    ...
    call_steps = [s for s in result.steps if s.kind.value == "call"]
